define(["require", "exports"], function (require, exports) {
    ...
    const ws_addr = document.getElementById('cfgwsip');
    ...
    const check_sound = document.getElementById('ck_sound');
    ...
    const com_conf = {
        mux: [
            document.getElementById('com_m0'),
            document.getElementById('com_m1'),
            document.getElementById('com_m2'),
            document.getElementById('com_m3'),
        ],
        frts: [
            document.getElementById('com_rs0'),
            document.getElementById('com_rs1'),
            document.getElementById('com_rs2'),
            document.getElementById('com_rs3'),
        ],
        crt: [
            document.getElementById('com_c0'),
            document.getElementById('com_c1'),
            document.getElementById('com_c2'),
        ],
        have_net: false,
        net: [],
        dev: [],
        ext: [
            { sel: document.getElementById('com_x0'), opts: [] },
            { sel: document.getElementById('com_x1'), opts: [] },
            { sel: document.getElementById('com_x2'), opts: [] },
            { sel: document.getElementById('com_x3'), opts: [] },
        ]
    };
    ...
    function run_core() {
        ...
        if (active_connection)
            active_connection.handle_tx_tick();
        if (run_ctl == 0) {
...
lv//////////////tXJzdf/Cd3n/cP//////tQ==`),
    ...
    const mclisting = [];
    function disk_image_luid(image) {
        if (image.identifier) {
    ...
    if (vt_use_offscreen) {
        ...
        vt_font_ctx = vt_font_canv.getContext('2d');
    }
    else {
        vt_use_offscreen = false;
        ...
        vt_font_ctx = vt_font_canv.getContext('2d', { willReadFrequently: true });
    }
    ...
    function compile_gl(c, flavor, src) {
        let s = c.createShader(flavor);
        if (s == null)
            throw new Error('ctx error 2');
    ...
    function init_gl(canv) {
        ...
        if (p == null)
            throw new Error('ctx error 3');
        let v = compile_gl(gl, gl.VERTEX_SHADER, `attribute vec2 p;\nattribute vec2 c;\nvarying vec2 t;\nuniform vec2 scn;\nvoid main() { t=c*vec2(0.015625,0.01953125)+vec2(0,-1); gl_Position=vec4(p.xy*scn+vec2(-1,1),0.0,1.0); }`);
        let f = compile_gl(gl, gl.FRAGMENT_SHADER, `precision mediump float;\nuniform sampler2D tx;\nvarying vec2 t;\nvoid main() { gl_FragColor = texture2D(tx, t); }`);
        gl.attachShader(p, v);
        gl.attachShader(p, f);
        gl.linkProgram(p);
    ...
    class VTerm {
        constructor(ident, container) {
            ...
            this.vbel_id = 0;
            this.vcontrol = false;
            const div_crt = container.children[0];
            const canv = container.children[1];
            if (canv.tagName != 'CANVAS')
                throw new Error(`invalid HTML for VTerm:${ident}`);
            ...
            if (glv) {
                ...
                if (!vt_use_offscreen || window.OffscreenCanvas == undefined) {
                    ...
                    this.octx = this.vt_ofs_canv.getContext('2d', { willReadFrequently: true });
                }
                ctx.fillStyle = '#6ac';
                ctx.strokeStyle = '#6ac';
            }
        ...
        set_cursor(col, row) {
            if (col == this.cursor_x && row == this.cursor_y)
                return;
        ...
        bind_dev(dev) {
            this.mux = dev;
            this.clear_buffer();
            this.set_cts((dev === null || dev === void 0 ? void 0 : dev.rts) || false);
        }
        can_receive() {
            return true;
        }
        set_cts(value) {
            ...
            for (let x = 0; x < s.length; x++)
                this.set_cell(VTerm.vt_limit + 72 + x, s.charCodeAt(x) | 0x100);
        }
        clear_buffer() {
    ...
    setInterval(function () {
        ...
        if (frame_crt1.show)
            cx_crt[1].update_blink();
        if (frame_crt2.show)
            cx_crt[2].update_blink();
        if (vt_font_bmp != null)
            vt_force_redraw = false;
    }, 125);
...
mHQBAgDKJ59tAQIatyeiY4BI34AAAAAAAAAA`);
    ...
    class DiskVol {
        constructor(image) {
            this.buf1 = new Uint8Array(400);
            this.buf_ual = new Uint8Array(400);
            this.name = '';
            this.status = '';
            this.guarded = true;
            this.sys_date = 0;
        ...
        read_media_sector(sector, buf) {
            let offset = sector * this.image.stride;
            for (let i = 0; i < 400; i++)
                buf[i] = this.image.data[offset + i];
        }
        read_ual_sectors(entry) {
            if (entry.ual)
                return entry.ual;
    ...
    function get_dirent_str(fe) {
        return `${fe.name} ${fe.ft.t} ${fe.t_len}${fe.s_len}${fe.fsi_str} ${hex(fe.cls)}${(fe.loadaddr != 0x8000) ? ' ' + hex(fe.loadaddr, 4) : ''} <${hex(fe.map_sect, 4)}:${hex(fe.map_index)}> ${fmt_disk_date(fe.date)}`;
    }
    class DiskViewer {
        constructor() {
            this.images = [];
            ...
            this.sel_disk = -1;
            this.sel_file = -1;
            const left_box = frame_disks.base.children[1];
            this.images_con = left_box.children[0].children[0];
            const listscrollbox = left_box.children[1];
            const datascrollbox = frame_disks.base.children[2];
            //this.listing_lproxy = listscrollbox.children[0] as HTMLDivElement;
            this.listingc = listscrollbox.children[0];
        ...
        on_list_click(index, ev) {
            if (this.sel_file == index)
                return;
            if (this.sel_file > -1) {
        ...
        drop_list_elems() {
            ...
            while (this.listingc.childElementCount > this.listing_index) {
                this.listingc.removeChild(this.listingc.lastChild);
            }
        }
        drop_datae_elems() {
        ...
        sector_list(file_id) {
            ...
            if (im_line == undefined || im_line.vol == undefined)
                return;
    ...
    class DSK2 {
        ...
        getlevel() { return 2; } // not used, but seems like a good number :)
        ...
        dma_step(ctrl) {
            ...
            if (mcsim.dma_end) {
                ctrl.end();
                this.busy_time = 10;
                if (this.dma_op == 2 && frame_disks.show)
                    disk_view_update = true;
            }
            else {
    ...
    class MockPrinter {
        ...
        get_dev() { return undefined; }
        bind_dev(dev) { }
        can_receive() { return true; }
        receive(v) {
            let c;
            v &= 127;
            if (v == 12) {
    ...
    class MUXPort {
        ...
        read_status() {
            return (this.write_busy ? 0 : 2) |
                (this.read_busy ? 1 : 0) |
                (this.overrun ? 16 : 0) |
                ((this._cts || this._force_cts) ? 0x20 : 0);
        }
        get_dev() {
            return this.line;
        }
    ...
    const oplist = [
        ...
        { n: 'SF', md: 0 /* OPM.IMPL */, fl: 1 /* FLAGS.F */, opc: function () { this.cc |= CC_F; } },
        { n: 'RF', md: 0 /* OPM.IMPL */, fl: 1 /* FLAGS.F */, opc: function () { this.cc &= CC_NF; } },
        { n: 'EI', md: 0 /* OPM.IMPL */, fl: 0 /* FLAGS.None */ },
        { n: 'DI', md: 0 /* OPM.IMPL */, fl: 0 /* FLAGS.None */ },
        { n: 'SL', md: 0 /* OPM.IMPL */, fl: 2 /* FLAGS.L */, opc: function () { this.cc |= CC_L; } },
        { n: 'RL', md: 0 /* OPM.IMPL */, fl: 2 /* FLAGS.L */, opc: function () { this.cc &= CC_NL; } },
        { n: ['CL', 'COML'], md: 0 /* OPM.IMPL */, fl: 2 /* FLAGS.L */, opc: function () { this.cc ^= CC_L; } },
        { n: ['RSR', 'RET'], md: 0 /* OPM.IMPL */, fl: 0 /* FLAGS.None */,
            opc: function (ctx) {
                this.pc = ctx.reg.x;
                this.set_rw(4 /* REG.X */, this.popw());
            } },
        { n: ['RI', 'RETI'], md: 0 /* OPM.IMPL */, fl: 4 /* FLAGS.FLMV */ },
        { n: '!ill0B', md: 0 /* OPM.IMPL */, fl: 0 /* FLAGS.None */ },
        { n: ['SYN', 'SYNC'], md: 0 /* OPM.IMPL */, fl: 0 /* FLAGS.None */, opc: function () { console.log('SYN'); } },
        ...
        // 0x2H
        { n: ['INRB', 'INC'], md: 11 /* OPM.RC */, w: 0 /* TXS.B */, i: 1, aop: 0 /* AOP.ADD */, fl: 5 /* FLAGS.ArithFMV */ },
        { n: ['DCRB', 'DEC'], md: 11 /* OPM.RC */, w: 0 /* TXS.B */, i: 1, aop: 1 /* AOP.SUB */, fl: 5 /* FLAGS.ArithFMV */ },
        { n: 'CLRB', md: 11 /* OPM.RC */, w: 0 /* TXS.B */, i: 0, aop: 7 /* AOP.CLR */, fl: 4 /* FLAGS.FLMV */ },
        { n: ['IVRB', 'NOT'], md: 11 /* OPM.RC */, w: 0 /* TXS.B */, aop: 6 /* AOP.NOT */, fl: 3 /* FLAGS.MV */ },
        { n: ['SRRB', 'SHR'], md: 11 /* OPM.RC */, w: 0 /* TXS.B */, i: 1, fl: 4 /* FLAGS.FLMV */,
            opf: function (ctx) {
                ctx.fault = 0;
                ctx.link = (ctx.srcval >> (8 - ctx.cdval)) & 1;
                return ctx.srcval << ctx.cdval;
            } },
        { n: ['SLRB', 'SHL'], md: 11 /* OPM.RC */, w: 0 /* TXS.B */, i: 1, fl: 4 /* FLAGS.FLMV */,
            opf: function (ctx) {
                const s = ctx.srcval;
        ...
        { n: ['RRRB', 'RORC'], md: 11 /* OPM.RC */, w: 0 /* TXS.B */, i: 1, fl: 4 /* FLAGS.FLMV */ },
        { n: ['RLRB', 'ROLC'], md: 11 /* OPM.RC */, w: 0 /* TXS.B */, i: 1, fl: 4 /* FLAGS.FLMV */ },
        { n: ['INAB', 'INC'], md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, dr: 1 /* REG.AL */, i: 1, aop: 0 /* AOP.ADD */, fl: 5 /* FLAGS.ArithFMV */ },
        { n: ['DCAB', 'DEC'], md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, dr: 1 /* REG.AL */, i: 1, aop: 1 /* AOP.SUB */, fl: 5 /* FLAGS.ArithFMV */ },
        { n: ['CLAB', 'CLR'], md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, dr: 1 /* REG.AL */, i: 0, aop: 7 /* AOP.CLR */, fl: 4 /* FLAGS.FLMV */ },
        { n: ['IVAB', 'NOT'], md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, dr: 1 /* REG.AL */, aop: 6 /* AOP.NOT */, fl: 3 /* FLAGS.MV */ },
        { n: ['SRAB', 'SHR'], md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, dr: 1 /* REG.AL */, fl: 4 /* FLAGS.FLMV */,
            opf: function (ctx) {
                ctx.fault = 0;
                ctx.link = ctx.srcval & 1;
                return (ctx.srcval >> 1) | (ctx.srcval & 128);
            } },
        { n: ['SLAB', 'SHL'], md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, dr: 1 /* REG.AL */, fl: 4 /* FLAGS.FLMV */,
            opf: function (ctx) {
                ctx.link = ctx.srcval >> 7;
        ...
        // 0x3H
        { n: ['INR', 'INC'], nm: 'INC', md: 11 /* OPM.RC */, w: 1 /* TXS.W */, i: 1, aop: 0 /* AOP.ADD */, fl: 5 /* FLAGS.ArithFMV */ },
        { n: ['DCR', 'DEC'], nm: 'DEC', md: 11 /* OPM.RC */, w: 1 /* TXS.W */, i: 1, aop: 1 /* AOP.SUB */, fl: 5 /* FLAGS.ArithFMV */ },
        { n: 'CLR', nm: 'CAD', md: 11 /* OPM.RC */, w: 1 /* TXS.W */, aop: 7 /* AOP.CLR */, fl: 4 /* FLAGS.FLMV */ },
        { n: ['IVR', 'NOT'], nm: ['IAD', 'NOT'], md: 11 /* OPM.RC */, w: 1 /* TXS.W */, aop: 6 /* AOP.NOT */, fl: 3 /* FLAGS.MV */ },
        { n: ['SRR', 'SHR'], nm: 'SHR', md: 11 /* OPM.RC */, w: 1 /* TXS.W */, i: 1, fl: 4 /* FLAGS.FLMV */,
            opf: function (ctx) {
                ctx.fault = 0;
                ctx.link = (ctx.srcval >> (16 - ctx.cdval)) & 1;
                return ctx.srcval << ctx.cdval;
            } },
        { n: ['SLR', 'SHL'], nm: 'SHL', md: 11 /* OPM.RC */, w: 1 /* TXS.W */, i: 1, fl: 4 /* FLAGS.FLMV */,
            opf: function (ctx) {
                const s = ctx.srcval;
        ...
        { n: ['RRR', 'RORC'], nm: ['RTR', 'RORC'], md: 11 /* OPM.RC */, w: 1 /* TXS.W */, i: 1, fl: 4 /* FLAGS.FLMV */ },
        { n: ['RLR', 'ROLC'], nm: ['RTL', 'ROLC'], md: 11 /* OPM.RC */, w: 1 /* TXS.W */, i: 1, fl: 4 /* FLAGS.FLMV */ },
        { n: ['INA', 'INC'], md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, dr: 0 /* REG.A */, i: 1, aop: 0 /* AOP.ADD */, fl: 5 /* FLAGS.ArithFMV */ },
        { n: ['DCA', 'DEC'], md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, dr: 0 /* REG.A */, i: 1, aop: 1 /* AOP.SUB */, fl: 5 /* FLAGS.ArithFMV */ },
        { n: ['CLA', 'CLR'], md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, dr: 0 /* REG.A */, i: 0, aop: 7 /* AOP.CLR */, fl: 4 /* FLAGS.FLMV */ },
        { n: ['IVA', 'NOT'], md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, dr: 0 /* REG.A */, aop: 6 /* AOP.NOT */, fl: 3 /* FLAGS.MV */ },
        { n: ['SRA', 'SHR'], md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, dr: 0 /* REG.A */, fl: 4 /* FLAGS.FLMV */,
            opf: function (ctx) {
                ctx.fault = 0;
                ctx.link = ctx.srcval & 1;
                return (ctx.srcval >> 1) | (ctx.srcval & 0x8000);
            } },
        { n: ['SLA', 'SHL'], md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, dr: 0 /* REG.A */, fl: 4 /* FLAGS.FLMV */,
            opf: function (ctx) {
                ctx.link = ctx.srcval >> 15;
                ctx.fault = (ctx.link ^ (ctx.srcval >> 14)) & 1;
                return ctx.srcval << 1;
            } },
        { n: ['INX', 'INC'], md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, dr: 4 /* REG.X */, i: 1, aop: 0 /* AOP.ADD */, fl: 5 /* FLAGS.ArithFMV */ },
        { n: ['DCX', 'DEC'], md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, dr: 4 /* REG.X */, i: 1, aop: 1 /* AOP.SUB */, fl: 5 /* FLAGS.ArithFMV */ },
        // 0x4H
        { n: 'ADDB', md: 7 /* OPM.RR */, w: 0 /* TXS.B */, aop: 0 /* AOP.ADD */, fl: 6 /* FLAGS.ArithFLMV */ },
        { n: 'SUBB', md: 7 /* OPM.RR */, w: 0 /* TXS.B */, aop: 1 /* AOP.SUB */, fl: 6 /* FLAGS.ArithFLMV */ },
        { n: 'ANDB', md: 7 /* OPM.RR */, w: 0 /* TXS.B */, aop: 3 /* AOP.AND */, fl: 3 /* FLAGS.MV */ },
        { n: ['ORIB', 'OR'], md: 7 /* OPM.RR */, w: 0 /* TXS.B */, aop: 4 /* AOP.ORI */, fl: 3 /* FLAGS.MV */ },
        { n: ['OREB', 'XOR'], md: 7 /* OPM.RR */, w: 0 /* TXS.B */, aop: 5 /* AOP.XOR */, fl: 3 /* FLAGS.MV */ },
        { n: ['XFRB', 'XFR'], md: 7 /* OPM.RR */, w: 0 /* TXS.B */, fl: 3 /* FLAGS.MV */, aop: 2 /* AOP.XFR */ },
        { n: ['', 'BIG.'], md: 0 /* OPM.IMPL */, fl: 4 /* FLAGS.FLMV */, x: 4 /* OPL_EXT.BIG */, xd: [
                ['A', 'S', 'C', 'ZAD', 'ZSU', 'M', 'D', 'DRM', 'CTB', 'CFB'],
                ['ADD', 'SUB', 'CMP', 'MOV', 'NEG', 'SMUL', 'DIV', 'DIVMOD', 'TOBIN', 'FMBIN'],
            ] },
        { n: ['', 'MEM.'], md: 0 /* OPM.IMPL */, fl: 4 /* FLAGS.FLMV */, x: 5 /* OPL_EXT.MEM */, xd: [
                [0, 1, 3, 2, 6, 6, 6, 6, 6, 6],
                ['CVX', 'CPV', 'MVV', 'SCN', 'MVF', 'ANC', 'ORC', 'XRC', 'CPF', 'FIL'],
                ['XREC', 'CMPC', 'MOVEC', 'SCANC', 'MOVEF', 'ANDC', 'ORC', 'XORC', 'CMPF', 'FILL'], //2
            ] },
        { n: ['AABB', 'ADD'], ms: 12 /* OPM.IMPL_R */, md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, sr: 1 /* REG.AL */, dr: 3 /* REG.BL */, aop: 0 /* AOP.ADD */, fl: 6 /* FLAGS.ArithFLMV */ },
        { n: ['SABB', 'SUB'], ms: 12 /* OPM.IMPL_R */, md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, sr: 1 /* REG.AL */, dr: 3 /* REG.BL */, aop: 1 /* AOP.SUB */, fl: 6 /* FLAGS.ArithFLMV */ },
        { n: ['NABB', 'AND'], ms: 12 /* OPM.IMPL_R */, md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, sr: 1 /* REG.AL */, dr: 3 /* REG.BL */, aop: 3 /* AOP.AND */, fl: 3 /* FLAGS.MV */ },
        { n: ['XAXB', 'XFR'], ms: 12 /* OPM.IMPL_R */, md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, sr: 1 /* REG.AL */, dr: 5 /* REG.XL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['XAYB', 'XFR'], ms: 12 /* OPM.IMPL_R */, md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, sr: 1 /* REG.AL */, dr: 7 /* REG.YL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['XABB', 'XFR'], ms: 12 /* OPM.IMPL_R */, md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, sr: 1 /* REG.AL */, dr: 3 /* REG.BL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['XAZB', 'XFR'], ms: 12 /* OPM.IMPL_R */, md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, sr: 1 /* REG.AL */, dr: 9 /* REG.ZL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['XASB', 'XFR'], ms: 12 /* OPM.IMPL_R */, md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, sr: 1 /* REG.AL */, dr: 11 /* REG.SL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        // 0x5H
        { n: 'ADD', md: 7 /* OPM.RR */, w: 1 /* TXS.W */, aop: 0 /* AOP.ADD */, fl: 6 /* FLAGS.ArithFLMV */ },
        { n: 'SUB', md: 7 /* OPM.RR */, w: 1 /* TXS.W */, aop: 1 /* AOP.SUB */, fl: 6 /* FLAGS.ArithFLMV */ },
        { n: 'AND', md: 7 /* OPM.RR */, w: 1 /* TXS.W */, aop: 3 /* AOP.AND */, fl: 3 /* FLAGS.MV */ },
        { n: ['ORI', 'OR'], md: 7 /* OPM.RR */, w: 1 /* TXS.W */, aop: 4 /* AOP.ORI */, fl: 3 /* FLAGS.MV */ },
        { n: ['ORE', 'XOR'], md: 7 /* OPM.RR */, w: 1 /* TXS.W */, aop: 5 /* AOP.XOR */, fl: 3 /* FLAGS.MV */ },
        { n: 'XFR', md: 10 /* OPM.RRS */, w: 1 /* TXS.W */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: 'EAO', md: 0 /* OPM.IMPL */, fl: 0 /* FLAGS.None */ },
        { n: 'DAO', md: 0 /* OPM.IMPL */, fl: 0 /* FLAGS.None */ },
        { n: ['AAB', 'ADD'], ms: 12 /* OPM.IMPL_R */, md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, sr: 0 /* REG.A */, dr: 2 /* REG.B */, aop: 0 /* AOP.ADD */, fl: 6 /* FLAGS.ArithFLMV */ },
        { n: ['SAB', 'SUB'], ms: 12 /* OPM.IMPL_R */, md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, sr: 0 /* REG.A */, dr: 2 /* REG.B */, aop: 1 /* AOP.SUB */, fl: 6 /* FLAGS.ArithFLMV */ },
        { n: ['NAB', 'AND'], ms: 12 /* OPM.IMPL_R */, md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, sr: 0 /* REG.A */, dr: 2 /* REG.B */, aop: 3 /* AOP.AND */, fl: 3 /* FLAGS.MV */ },
        { n: ['XAX', 'XFR'], ms: 12 /* OPM.IMPL_R */, md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, sr: 0 /* REG.A */, dr: 4 /* REG.X */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['XAY', 'XFR'], ms: 12 /* OPM.IMPL_R */, md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, sr: 0 /* REG.A */, dr: 6 /* REG.Y */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['XAB', 'XFR'], ms: 12 /* OPM.IMPL_R */, md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, sr: 0 /* REG.A */, dr: 2 /* REG.B */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['XAZ', 'XFR'], ms: 12 /* OPM.IMPL_R */, md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, sr: 0 /* REG.A */, dr: 8 /* REG.Z */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['XAS', 'XFR'], ms: 12 /* OPM.IMPL_R */, md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, sr: 0 /* REG.A */, dr: 10 /* REG.S */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        // 0x6H
        { n: ['LDX', 'LD'], ms: 1 /* OPM.IMM */, w: 1 /* TXS.W */, md: 12 /* OPM.IMPL_R */, dr: 4 /* REG.X */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDX', 'LD'], ms: 2 /* OPM.DIR */, w: 1 /* TXS.W */, md: 12 /* OPM.IMPL_R */, dr: 4 /* REG.X */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDX', 'LD'], ms: 3 /* OPM.IND */, w: 1 /* TXS.W */, md: 12 /* OPM.IMPL_R */, dr: 4 /* REG.X */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDX', 'LD'], ms: 4 /* OPM.PCO */, w: 1 /* TXS.W */, md: 12 /* OPM.IMPL_R */, dr: 4 /* REG.X */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDX', 'LD'], ms: 5 /* OPM.IPO */, w: 1 /* TXS.W */, md: 12 /* OPM.IMPL_R */, dr: 4 /* REG.X */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDX', 'LD'], ms: 6 /* OPM.MOD */, w: 1 /* TXS.W */, md: 12 /* OPM.IMPL_R */, dr: 4 /* REG.X */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['SVC', 'SVCALL'], ms: 1 /* OPM.IMM */, w: 0 /* TXS.B */, fl: 0 /* FLAGS.None */, md: 0 /* OPM.IMPL */,
            opc: function (ctx) { this.handle_svc(ctx.srcval); } },
        { n: ['', 'MEM.'], md: 0 /* OPM.IMPL */, fl: 4 /* FLAGS.FLMV */, x: 5 /* OPL_EXT.MEM */, sr: 0 /* REG.A */, xd: [
                [0, 1, 3, 2, 6, 6, 6, 6, 6, 6],
                ['CVXR', 'CPVR', 'MVVR', 'SCNR', 'MVFR', 'ANCR', 'ORCR', 'XRCR', 'CPFR', 'FILR'],
                ['XREC', 'CMPC', 'MOVEC', 'SCANC', 'MOVEF', 'ANDC', 'ORC', 'XORC', 'CMPF', 'FILL'], //2
            ] },
        { n: ['STX', 'ST'], md: 1 /* OPM.IMM */, ms: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, sr: 4 /* REG.X */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STX', 'ST'], md: 2 /* OPM.DIR */, ms: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, sr: 4 /* REG.X */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STX', 'ST'], md: 3 /* OPM.IND */, ms: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, sr: 4 /* REG.X */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STX', 'ST'], md: 4 /* OPM.PCO */, ms: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, sr: 4 /* REG.X */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STX', 'ST'], md: 5 /* OPM.IPO */, ms: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, sr: 4 /* REG.X */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STX', 'ST'], md: 6 /* OPM.MOD */, ms: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, sr: 4 /* REG.X */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LST', 'LDST'], md: 2 /* OPM.DIR */, w: 0 /* TXS.B */, fl: 6 /* FLAGS.ArithFLMV */ },
        { n: ['SST', 'STST'], md: 0 /* OPM.IMPL */, ms: 2 /* OPM.DIR */, w: 0 /* TXS.B */, fl: 6 /* FLAGS.ArithFLMV */ },
        // 0x7H
        { n: '!ill70', md: 1 /* OPM.IMM */, w: 2 /* TXS.F */, fl: 0 /* FLAGS.None */ },
        { n: 'JMP', md: 2 /* OPM.DIR */, w: 2 /* TXS.F */, fl: 0 /* FLAGS.None */, opc: function (ctx) { this.pc = ctx.addr; } },
        { n: 'JMP', md: 3 /* OPM.IND */, w: 2 /* TXS.F */, fl: 0 /* FLAGS.None */, opc: function (ctx) { this.pc = ctx.addr; } },
        { n: 'JMP', md: 4 /* OPM.PCO */, w: 2 /* TXS.F */, fl: 0 /* FLAGS.None */, opc: function (ctx) { this.pc = ctx.addr; } },
        { n: 'JMP', md: 5 /* OPM.IPO */, w: 2 /* TXS.F */, fl: 0 /* FLAGS.None */, opc: function (ctx) { this.pc = ctx.addr; } },
        { n: 'JMP', md: 6 /* OPM.MOD */, w: 2 /* TXS.F */, fl: 0 /* FLAGS.None */, opc: function (ctx) { this.pc = ctx.addr; } },
        { n: ['EPE', 'EN.PEF'], md: 0 /* OPM.IMPL */, fl: 0 /* FLAGS.None */ },
        { n: 'MUL', md: 8 /* OPM.RRX */, w: 1 /* TXS.W */, fl: 6 /* FLAGS.ArithFLMV */ },
        { n: 'DIV', md: 8 /* OPM.RRX */, w: 1 /* TXS.W */, fl: 6 /* FLAGS.ArithFLMV */ },
        { n: ['JSR', 'CALL'], md: 2 /* OPM.DIR */, w: 2 /* TXS.F */, fl: 0 /* FLAGS.None */, opc: function (ctx) { this.f_jsr(ctx.addr); } },
        { n: ['JSR', 'CALL'], md: 3 /* OPM.IND */, w: 2 /* TXS.F */, fl: 0 /* FLAGS.None */, opc: function (ctx) { this.f_jsr(ctx.addr); } },
        { n: ['JSR', 'CALL'], md: 4 /* OPM.PCO */, w: 2 /* TXS.F */, fl: 0 /* FLAGS.None */, opc: function (ctx) { this.f_jsr(ctx.addr); } },
        { n: ['JSR', 'CALL'], md: 5 /* OPM.IPO */, w: 2 /* TXS.F */, fl: 0 /* FLAGS.None */, opc: function (ctx) { this.f_jsr(ctx.addr); } },
        { n: ['JSR', 'CALL'], md: 6 /* OPM.MOD */, w: 2 /* TXS.F */, fl: 0 /* FLAGS.None */, opc: function (ctx) { this.f_jsr(ctx.addr); } },
        { n: ['STK', 'MPUSH'], md: 11 /* OPM.RC */, w: 2 /* TXS.F */, i: 1, x: 0 /* OPL_EXT.MPUSH */, fl: 0 /* FLAGS.None */, opc: function (ctx) {
                let s = this.get_rw(10 /* REG.S */);
                ...
                this.set_rw(10 /* REG.S */, s & 65535);
            } },
        { n: ['POP', 'MPOP'], md: 11 /* OPM.RC */, w: 2 /* TXS.F */, i: 1, x: 1 /* OPL_EXT.MPOP */, fl: 0 /* FLAGS.None */, opc: function (ctx) {
                let s = this.get_rw(10 /* REG.S */);
                while (ctx.cdval > 0) {
        ...
        // 0x8H
        { n: ['LDAB', 'LD'], ms: 1 /* OPM.IMM */, md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, dr: 1 /* REG.AL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDAB', 'LD'], ms: 2 /* OPM.DIR */, md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, dr: 1 /* REG.AL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDAB', 'LD'], ms: 3 /* OPM.IND */, md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, dr: 1 /* REG.AL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDAB', 'LD'], ms: 4 /* OPM.PCO */, md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, dr: 1 /* REG.AL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDAB', 'LD'], ms: 5 /* OPM.IPO */, md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, dr: 1 /* REG.AL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDAB', 'LD'], ms: 6 /* OPM.MOD */, md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, dr: 1 /* REG.AL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['DPE', 'DIS.PEF'], md: 0 /* OPM.IMPL */, fl: 0 /* FLAGS.None */ },
        { n: '!ill87', md: 0 /* OPM.IMPL */, fl: 0 /* FLAGS.None */ },
        { n: ['LDAB+ A', 'LD'], ms: 13 /* OPM.IMPL_R_DIR */, md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, sr: 0 /* REG.A */, dr: 1 /* REG.AL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDAB+ B', 'LD'], ms: 13 /* OPM.IMPL_R_DIR */, md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, sr: 2 /* REG.B */, dr: 1 /* REG.AL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDAB+ X', 'LD'], ms: 13 /* OPM.IMPL_R_DIR */, md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, sr: 4 /* REG.X */, dr: 1 /* REG.AL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDAB+ Y', 'LD'], ms: 13 /* OPM.IMPL_R_DIR */, md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, sr: 6 /* REG.Y */, dr: 1 /* REG.AL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDAB+ Z', 'LD'], ms: 13 /* OPM.IMPL_R_DIR */, md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, sr: 8 /* REG.Z */, dr: 1 /* REG.AL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDAB+ S', 'LD'], ms: 13 /* OPM.IMPL_R_DIR */, md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, sr: 10 /* REG.S */, dr: 1 /* REG.AL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDAB+ C', 'LD'], ms: 13 /* OPM.IMPL_R_DIR */, md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, sr: 12 /* REG.C */, dr: 1 /* REG.AL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDAB+ P', 'LD'], ms: 13 /* OPM.IMPL_R_DIR */, md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, sr: 14 /* REG.P */, dr: 1 /* REG.AL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        // 0x9H
        { n: ['LDA', 'LD'], ms: 1 /* OPM.IMM */, md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, dr: 0 /* REG.A */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDA', 'LD'], ms: 2 /* OPM.DIR */, md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, dr: 0 /* REG.A */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDA', 'LD'], ms: 3 /* OPM.IND */, md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, dr: 0 /* REG.A */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDA', 'LD'], ms: 4 /* OPM.PCO */, md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, dr: 0 /* REG.A */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDA', 'LD'], ms: 5 /* OPM.IPO */, md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, dr: 0 /* REG.A */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDA', 'LD'], ms: 6 /* OPM.MOD */, md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, dr: 0 /* REG.A */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['SOP', 'SET.PO'], md: 0 /* OPM.IMPL */, fl: 0 /* FLAGS.None */ },
        { n: '!ill97', md: 0 /* OPM.IMPL */, fl: 0 /* FLAGS.None */ },
        { n: ['LDA+ A', 'LD'], ms: 13 /* OPM.IMPL_R_DIR */, md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, sr: 0 /* REG.A */, dr: 0 /* REG.A */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDA+ B', 'LD'], ms: 13 /* OPM.IMPL_R_DIR */, md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, sr: 2 /* REG.B */, dr: 0 /* REG.A */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDA+ X', 'LD'], ms: 13 /* OPM.IMPL_R_DIR */, md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, sr: 4 /* REG.X */, dr: 0 /* REG.A */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDA+ Y', 'LD'], ms: 13 /* OPM.IMPL_R_DIR */, md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, sr: 6 /* REG.Y */, dr: 0 /* REG.A */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDA+ Z', 'LD'], ms: 13 /* OPM.IMPL_R_DIR */, md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, sr: 8 /* REG.Z */, dr: 0 /* REG.A */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDA+ S', 'LD'], ms: 13 /* OPM.IMPL_R_DIR */, md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, sr: 10 /* REG.S */, dr: 0 /* REG.A */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDA+ C', 'LD'], ms: 13 /* OPM.IMPL_R_DIR */, md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, sr: 12 /* REG.C */, dr: 0 /* REG.A */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDA+ P', 'LD'], ms: 13 /* OPM.IMPL_R_DIR */, md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, sr: 14 /* REG.P */, dr: 0 /* REG.A */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        // 0xAH
        { n: ['STAB', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 1 /* OPM.IMM */, w: 0 /* TXS.B */, sr: 1 /* REG.AL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STAB', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 2 /* OPM.DIR */, w: 0 /* TXS.B */, sr: 1 /* REG.AL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STAB', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 3 /* OPM.IND */, w: 0 /* TXS.B */, sr: 1 /* REG.AL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STAB', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 4 /* OPM.PCO */, w: 0 /* TXS.B */, sr: 1 /* REG.AL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STAB', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 5 /* OPM.IPO */, w: 0 /* TXS.B */, sr: 1 /* REG.AL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STAB', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 6 /* OPM.MOD */, w: 0 /* TXS.B */, sr: 1 /* REG.AL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['SEP', 'SET.PE'], md: 0 /* OPM.IMPL */, fl: 0 /* FLAGS.None */ },
        { n: '!illA7', md: 0 /* OPM.IMPL */, fl: 0 /* FLAGS.None */ },
        { n: ['STAB+ A', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 13 /* OPM.IMPL_R_DIR */, w: 0 /* TXS.B */, sr: 1 /* REG.AL */, dr: 0 /* REG.A */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STAB+ B', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 13 /* OPM.IMPL_R_DIR */, w: 0 /* TXS.B */, sr: 1 /* REG.AL */, dr: 2 /* REG.B */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STAB+ X', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 13 /* OPM.IMPL_R_DIR */, w: 0 /* TXS.B */, sr: 1 /* REG.AL */, dr: 4 /* REG.X */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STAB+ Y', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 13 /* OPM.IMPL_R_DIR */, w: 0 /* TXS.B */, sr: 1 /* REG.AL */, dr: 6 /* REG.Y */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STAB+ Z', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 13 /* OPM.IMPL_R_DIR */, w: 0 /* TXS.B */, sr: 1 /* REG.AL */, dr: 8 /* REG.Z */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STAB+ S', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 13 /* OPM.IMPL_R_DIR */, w: 0 /* TXS.B */, sr: 1 /* REG.AL */, dr: 10 /* REG.S */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STAB+ C', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 13 /* OPM.IMPL_R_DIR */, w: 0 /* TXS.B */, sr: 1 /* REG.AL */, dr: 12 /* REG.C */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STAB+ P', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 13 /* OPM.IMPL_R_DIR */, w: 0 /* TXS.B */, sr: 1 /* REG.AL */, dr: 14 /* REG.P */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        // 0xBH
        { n: ['STA', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 1 /* OPM.IMM */, w: 1 /* TXS.W */, sr: 0 /* REG.A */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STA', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 2 /* OPM.DIR */, w: 1 /* TXS.W */, sr: 0 /* REG.A */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STA', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 3 /* OPM.IND */, w: 1 /* TXS.W */, sr: 0 /* REG.A */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STA', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 4 /* OPM.PCO */, w: 1 /* TXS.W */, sr: 0 /* REG.A */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STA', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 5 /* OPM.IPO */, w: 1 /* TXS.W */, sr: 0 /* REG.A */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STA', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 6 /* OPM.MOD */, w: 1 /* TXS.W */, sr: 0 /* REG.A */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['ECK', 'EN.CLK'], md: 0 /* OPM.IMPL */, fl: 0 /* FLAGS.None */ },
        { n: '!illB7', md: 0 /* OPM.IMPL */, fl: 0 /* FLAGS.None */ },
        { n: ['STA+ A', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 13 /* OPM.IMPL_R_DIR */, w: 1 /* TXS.W */, sr: 0 /* REG.A */, dr: 0 /* REG.A */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STA+ B', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 13 /* OPM.IMPL_R_DIR */, w: 1 /* TXS.W */, sr: 0 /* REG.A */, dr: 2 /* REG.B */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STA+ X', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 13 /* OPM.IMPL_R_DIR */, w: 1 /* TXS.W */, sr: 0 /* REG.A */, dr: 4 /* REG.X */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STA+ Y', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 13 /* OPM.IMPL_R_DIR */, w: 1 /* TXS.W */, sr: 0 /* REG.A */, dr: 6 /* REG.Y */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STA+ Z', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 13 /* OPM.IMPL_R_DIR */, w: 1 /* TXS.W */, sr: 0 /* REG.A */, dr: 8 /* REG.Z */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STA+ S', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 13 /* OPM.IMPL_R_DIR */, w: 1 /* TXS.W */, sr: 0 /* REG.A */, dr: 10 /* REG.S */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STA+ C', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 13 /* OPM.IMPL_R_DIR */, w: 1 /* TXS.W */, sr: 0 /* REG.A */, dr: 12 /* REG.C */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STA+ P', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 13 /* OPM.IMPL_R_DIR */, w: 1 /* TXS.W */, sr: 0 /* REG.A */, dr: 14 /* REG.P */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        // 0xCH
        { n: ['LDBB', 'LD'], ms: 1 /* OPM.IMM */, md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, dr: 3 /* REG.BL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDBB', 'LD'], ms: 2 /* OPM.DIR */, md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, dr: 3 /* REG.BL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDBB', 'LD'], ms: 3 /* OPM.IND */, md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, dr: 3 /* REG.BL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDBB', 'LD'], ms: 4 /* OPM.PCO */, md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, dr: 3 /* REG.BL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDBB', 'LD'], ms: 5 /* OPM.IPO */, md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, dr: 3 /* REG.BL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDBB', 'LD'], ms: 6 /* OPM.MOD */, md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, dr: 3 /* REG.BL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['DCK', 'DIS.CLK'], md: 0 /* OPM.IMPL */, fl: 0 /* FLAGS.None */ },
        { n: '!illC7', md: 0 /* OPM.IMPL */, fl: 0 /* FLAGS.None */ },
        { n: ['LDBB+ A', 'LD'], ms: 13 /* OPM.IMPL_R_DIR */, md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, sr: 0 /* REG.A */, dr: 3 /* REG.BL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDBB+ B', 'LD'], ms: 13 /* OPM.IMPL_R_DIR */, md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, sr: 2 /* REG.B */, dr: 3 /* REG.BL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDBB+ X', 'LD'], ms: 13 /* OPM.IMPL_R_DIR */, md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, sr: 4 /* REG.X */, dr: 3 /* REG.BL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDBB+ Y', 'LD'], ms: 13 /* OPM.IMPL_R_DIR */, md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, sr: 6 /* REG.Y */, dr: 3 /* REG.BL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDBB+ Z', 'LD'], ms: 13 /* OPM.IMPL_R_DIR */, md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, sr: 8 /* REG.Z */, dr: 3 /* REG.BL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDBB+ S', 'LD'], ms: 13 /* OPM.IMPL_R_DIR */, md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, sr: 10 /* REG.S */, dr: 3 /* REG.BL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDBB+ C', 'LD'], ms: 13 /* OPM.IMPL_R_DIR */, md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, sr: 12 /* REG.C */, dr: 3 /* REG.BL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDBB+ P', 'LD'], ms: 13 /* OPM.IMPL_R_DIR */, md: 12 /* OPM.IMPL_R */, w: 0 /* TXS.B */, sr: 14 /* REG.P */, dr: 3 /* REG.BL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        // 0xDH
        { n: ['LDB', 'LD'], ms: 1 /* OPM.IMM */, md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, dr: 2 /* REG.B */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDB', 'LD'], ms: 2 /* OPM.DIR */, md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, dr: 2 /* REG.B */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDB', 'LD'], ms: 3 /* OPM.IND */, md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, dr: 2 /* REG.B */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDB', 'LD'], ms: 4 /* OPM.PCO */, md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, dr: 2 /* REG.B */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDB', 'LD'], ms: 5 /* OPM.IPO */, md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, dr: 2 /* REG.B */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDB', 'LD'], ms: 6 /* OPM.MOD */, md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, dr: 2 /* REG.B */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: 'STR', w: 1 /* TXS.W */, md: 9 /* OPM.RRSR */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: 'SAR', ms: 12 /* OPM.IMPL_R */, sr: 0 /* REG.A */, w: 1 /* TXS.W */, md: 0 /* OPM.IMPL */, x: 6 /* OPL_EXT.RII */, fl: 6 /* FLAGS.ArithFLMV */ },
        { n: ['LDB+ A', 'LD'], ms: 13 /* OPM.IMPL_R_DIR */, md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, sr: 0 /* REG.A */, dr: 2 /* REG.B */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDB+ B', 'LD'], ms: 13 /* OPM.IMPL_R_DIR */, md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, sr: 2 /* REG.B */, dr: 2 /* REG.B */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDB+ X', 'LD'], ms: 13 /* OPM.IMPL_R_DIR */, md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, sr: 4 /* REG.X */, dr: 2 /* REG.B */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDB+ Y', 'LD'], ms: 13 /* OPM.IMPL_R_DIR */, md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, sr: 6 /* REG.Y */, dr: 2 /* REG.B */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDB+ Z', 'LD'], ms: 13 /* OPM.IMPL_R_DIR */, md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, sr: 8 /* REG.Z */, dr: 2 /* REG.B */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDB+ S', 'LD'], ms: 13 /* OPM.IMPL_R_DIR */, md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, sr: 10 /* REG.S */, dr: 2 /* REG.B */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDB+ C', 'LD'], ms: 13 /* OPM.IMPL_R_DIR */, md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, sr: 12 /* REG.C */, dr: 2 /* REG.B */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['LDB+ P', 'LD'], ms: 13 /* OPM.IMPL_R_DIR */, md: 12 /* OPM.IMPL_R */, w: 1 /* TXS.W */, sr: 14 /* REG.P */, dr: 2 /* REG.B */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        // 0xEH
        { n: ['STBB', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 1 /* OPM.IMM */, w: 0 /* TXS.B */, sr: 3 /* REG.BL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STBB', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 2 /* OPM.DIR */, w: 0 /* TXS.B */, sr: 3 /* REG.BL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STBB', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 3 /* OPM.IND */, w: 0 /* TXS.B */, sr: 3 /* REG.BL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STBB', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 4 /* OPM.PCO */, w: 0 /* TXS.B */, sr: 3 /* REG.BL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STBB', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 5 /* OPM.IPO */, w: 0 /* TXS.B */, sr: 3 /* REG.BL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STBB', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 6 /* OPM.MOD */, w: 0 /* TXS.B */, sr: 3 /* REG.BL */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: 'LAR', w: 1 /* TXS.W */, md: 12 /* OPM.IMPL_R */, dr: 0 /* REG.A */, i: 1, x: 6 /* OPL_EXT.RII */, fl: 6 /* FLAGS.ArithFLMV */ },
        { n: '!illE7', md: 0 /* OPM.IMPL */, fl: 0 /* FLAGS.None */ },
        { n: ['STBB+ A', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 13 /* OPM.IMPL_R_DIR */, w: 0 /* TXS.B */, sr: 3 /* REG.BL */, dr: 0 /* REG.A */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STBB+ B', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 13 /* OPM.IMPL_R_DIR */, w: 0 /* TXS.B */, sr: 3 /* REG.BL */, dr: 2 /* REG.B */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STBB+ X', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 13 /* OPM.IMPL_R_DIR */, w: 0 /* TXS.B */, sr: 3 /* REG.BL */, dr: 4 /* REG.X */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STBB+ Y', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 13 /* OPM.IMPL_R_DIR */, w: 0 /* TXS.B */, sr: 3 /* REG.BL */, dr: 6 /* REG.Y */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STBB+ Z', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 13 /* OPM.IMPL_R_DIR */, w: 0 /* TXS.B */, sr: 3 /* REG.BL */, dr: 8 /* REG.Z */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STBB+ S', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 13 /* OPM.IMPL_R_DIR */, w: 0 /* TXS.B */, sr: 3 /* REG.BL */, dr: 10 /* REG.S */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STBB+ C', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 13 /* OPM.IMPL_R_DIR */, w: 0 /* TXS.B */, sr: 3 /* REG.BL */, dr: 12 /* REG.C */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STBB+ P', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 13 /* OPM.IMPL_R_DIR */, w: 0 /* TXS.B */, sr: 3 /* REG.BL */, dr: 14 /* REG.P */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        // 0xFH
        { n: ['STB', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 1 /* OPM.IMM */, w: 1 /* TXS.W */, sr: 2 /* REG.B */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STB', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 2 /* OPM.DIR */, w: 1 /* TXS.W */, sr: 2 /* REG.B */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STB', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 3 /* OPM.IND */, w: 1 /* TXS.W */, sr: 2 /* REG.B */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STB', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 4 /* OPM.PCO */, w: 1 /* TXS.W */, sr: 2 /* REG.B */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STB', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 5 /* OPM.IPO */, w: 1 /* TXS.W */, sr: 2 /* REG.B */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STB', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 6 /* OPM.MOD */, w: 1 /* TXS.W */, sr: 2 /* REG.B */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: 'LIO', md: 0 /* OPM.IMPL */, x: 7 /* OPL_EXT.XIO */, fl: 0 /* FLAGS.None */ },
        { n: ['MVL', 'MOVML'], md: 0 /* OPM.IMPL */, w: 1 /* TXS.W */, sr: 2 /* REG.B */, dr: 6 /* REG.Y */, fl: 0 /* FLAGS.None */ },
        { n: ['STB+ A', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 13 /* OPM.IMPL_R_DIR */, w: 1 /* TXS.W */, sr: 2 /* REG.B */, dr: 0 /* REG.A */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STB+ B', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 13 /* OPM.IMPL_R_DIR */, w: 1 /* TXS.W */, sr: 2 /* REG.B */, dr: 2 /* REG.B */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STB+ X', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 13 /* OPM.IMPL_R_DIR */, w: 1 /* TXS.W */, sr: 2 /* REG.B */, dr: 4 /* REG.X */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STB+ Y', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 13 /* OPM.IMPL_R_DIR */, w: 1 /* TXS.W */, sr: 2 /* REG.B */, dr: 6 /* REG.Y */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STB+ Z', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 13 /* OPM.IMPL_R_DIR */, w: 1 /* TXS.W */, sr: 2 /* REG.B */, dr: 8 /* REG.Z */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STB+ S', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 13 /* OPM.IMPL_R_DIR */, w: 1 /* TXS.W */, sr: 2 /* REG.B */, dr: 10 /* REG.S */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STB+ C', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 13 /* OPM.IMPL_R_DIR */, w: 1 /* TXS.W */, sr: 2 /* REG.B */, dr: 12 /* REG.C */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
        { n: ['STB+ P', 'ST'], ms: 12 /* OPM.IMPL_R */, md: 13 /* OPM.IMPL_R_DIR */, w: 1 /* TXS.W */, sr: 2 /* REG.B */, dr: 14 /* REG.P */, aop: 2 /* AOP.XFR */, fl: 3 /* FLAGS.MV */ },
    ];
    ...
    function softlog(text) {
        ...
        (_a = editors.output1) === null || _a === void 0 ? void 0 : _a.revealLineInCenter(line);
    }
    ...
    class UMCPU6 {
        ...
        flags_n8vmfl(old, res) {
            old &= 255;
            res &= 255;
            this.mk_reasonb(0, res, 1 /* ARITH_OP.ADD */);
        ...
        flags_n16vmfl(old, res) {
            old &= 65535;
            res &= 65535;
            this.mk_reasonw(0, res, 1 /* ARITH_OP.ADD */);
        ...
        lea_mod(sz = 1) {
            ...
            if ((modv & 4) != 0)
                sz = 2; // indirection always points at words
            if ((modv & 3) == 2) {
                // reg predec
                reg = (reg - sz) & 65535;
            }
        ...
        step() {
            ...
            switch (opl.md) {
                ...
                case 5 /* OPM.IPO */:
                    ...
                    break;
                case 6 /* OPM.MOD */:
                    this.lea_mod((opl.w == 0 /* TXS.B */) ? 1 : 2);
                    break;
                case 7 /* OPM.RR */:
                    if (opl.w == 0 /* TXS.B */)
                        this.rrfetchb();
                ...
                case 12 /* OPM.IMPL_R */:
                    ...
                    if (opl.ms == undefined) {
                        ...
                        if (opl.i !== undefined)
                            rref.cdval += opl.i;
                        if (opl.w == 0 /* TXS.B */)
                            rref.srcval = this.get_rb(rref.dstreg);
                        else
                            rref.srcval = this.get_rw(rref.dstreg);
                    }
            ...
            if (opl.opf != undefined) {
                ...
                switch (opl.fl) {
                    ...
                    case 3 /* FLAGS.MV */:
                        if (opl.w == 0 /* TXS.B */)
                            rref.cdval = this.flags_8vm(fr);
                        else
                            rref.cdval = this.flags_16vm(fr);
                        break;
                    case 4 /* FLAGS.FLMV */:
                        if (opl.w == 0 /* TXS.B */)
                            rref.cdval = this.flags_8vmfl(fr, rref.fault, rref.link);
                        else
                            rref.cdval = this.flags_16vmfl(fr, rref.fault, rref.link);
                        break;
                    case 5 /* FLAGS.ArithFMV */:
                    case 6 /* FLAGS.ArithFLMV */:
                        if (opl.w == 0 /* TXS.B */)
                ...
                if (opl.w == 0 /* TXS.B */)
                    ...
                    softlog(`${hex(op)}: OPF(${hex(fr, 4)})->(${hex(rref.cdval, 4)})`);
                if (opl.md == 0 /* OPM.IMPL */)
                    softlog(`warn: (${hex(op)}) OPM.IMPL with OPF`);
            }
                ...
                switch (opl.aop) {
                    ...
                    case 2 /* AOP.XFR */:
                        if (opl.w == 0 /* TXS.B */)
                            rref.cdval = this.flags_8vm(rref.srcval);
                        else
                            rref.cdval = this.flags_16vm(rref.srcval);
                    ...
                    case 3 /* AOP.AND */:
                        if (opl.w == 0 /* TXS.B */)
                            ...
                            rref.cdval = this.flags_n16vmfl(rref.cdval, rref.srcval & rref.cdval);
                        break;
                    case 4 /* AOP.ORI */:
                        if (opl.w == 0 /* TXS.B */)
                            rref.cdval = this.flags_n8vmfl(rref.cdval, rref.srcval | rref.cdval);
                    ...
                    case 6 /* AOP.NOT */:
                        if (opl.w == 0 /* TXS.B */)
                            ...
                            rref.cdval = this.flags_16vm(rref.srcval ^ 65535);
                        break;
                    case 7 /* AOP.CLR */:
                        if (opl.w == 0 /* TXS.B */)
                            rref.cdval = this.flags_8vmfl(rref.cdval, 0, 0);
                        else
                            rref.cdval = this.flags_16vmfl(rref.cdval, 0, 0);
                        break;
                }
            }
            else if (opl.x == 5 /* OPL_EXT.MEM */) { // MEM.
                const ib = this.pcfetch();
                const sel = ib >> 4;
                let iml;
                if (opl.sr != undefined)
                    iml = this.levelreg.al;
                else
                    iml = this.pcfetch();
                switch (sel) {
                    case 0: /*CVX*/ break;
            ...
            switch (opl.md) { // generic RMW write backs
                ...
                case 13 /* OPM.IMPL_R_DIR */:
                    if (opl.w == 0 /* TXS.B */)
                        this.write(rref.addr, rref.cdval);
                    else
                ...
                case 9 /* OPM.RRSR */: // STR
                    if (opl.w == 0 /* TXS.B */) {
                        ...
                        this.set_rw(rref.dstreg, rref.cdval);
                    return;
                case 12 /* OPM.IMPL_R */:
                    if (opl.w == 0 /* TXS.B */)
                        this.set_rb(rref.dstreg, rref.cdval);
    ...
    class CPU6 {
        ...
        disassembly(address, ret) {
            ...
            let rsh = 0;
            if (opcs.w == 1 /* TXS.W */ || opcs.w == 2 /* TXS.F */) {
                rsh = 1;
            }
            let regdst = 0;
            let regsrc = 0;
            ...
            extenc: switch (opcs.x) {
                ...
                case 6 /* OPL_EXT.RII */: { // LAR/SAR
                    const rvc = dfetch();
                    const level = rvc >> 4;
                    if (opcs.i == 1)
                        pstr = `${level},${regconv[1][rvc & 15]}`;
                    else
                        psstr = `${level},${regconv[1][rvc & 15]}`;
                    break;
                }
                case 7 /* OPL_EXT.XIO */: {
                    const rvc = dfetch();
                    ...
                    if ((rvc & 1) != 0) {
                        ...
                        if (xiobyte && (name_conv != 1 /* NAMECON.MEI */))
                            opname += 'B';
                        psstr = pstr;
                        ...
                        if (is_cent) {
                            ...
                            psstr = regconv[xiobyte ? 0 : 1][regh];
                    }
                    break;
                }
    ...
    function init_serial_ui() {
        ...
        for (let i = 0; i < 4; i++) {
            ...
            com_conf.frts[i].checked = false;
            com_conf.frts[i].addEventListener('change', serial_ui_change);
        }
    }
    ...
    function update_serial_config() {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            mux_port._force_cts = com_conf.frts[i].checked;
            const crt_port = cx_crt[i];
            const mux_to_crt = !ext_avail || ((com_conf.mux[i].value == '0') &&
                (com_conf.crt[i].value == '0'));
            let mux_target;
            let crt_target;
            let ext_target;
            ...
            if (mux_target != mux_port.get_dev())
                mux_port.bind_dev(mux_target);
            if (crt_target != crt_port.get_dev())
                crt_port.bind_dev(crt_target);
            if (ext_port && ext_target) {
                ext_port.dev_count++;
                if (ext_port.get_dev() != ext_target)
                    ext_port.bind_dev(ext_target);
            }
        }
        if (com_conf.mux[3].value == '0') {
            ...
            if (!ext_avail) {
                ...
                mux_target = ext_port;
                ext_port.dev_count++;
                if (ext_port.get_dev() != mux_port)
                    ext_port.bind_dev(mux_port);
            }
            if (mux_target != mux_port.get_dev())
                mux_port.bind_dev(mux_target);
        }
    ...
    run_button.addEventListener('click', function (ev) {
        if (check_sound.checked && wa == undefined)
            wa = wa_setup();
