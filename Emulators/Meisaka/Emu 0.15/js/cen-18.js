...
lv//////////////tXJzdf/Cd3n/cP//////tQ==`),
    ...
    const vt_font_bmp = [null, null, null];
    let font_load = (async function () {
        update_font_data(0xf0aa66);
        vt_font_bmp[0] = await createImageBitmap(vt_font_data);
        update_font_data(0xc0aaaa);
        vt_font_bmp[1] = await createImageBitmap(vt_font_data);
        update_font_data(0x147ae0);
        vt_font_bmp[2] = await createImageBitmap(vt_font_data);
    })();
    const vtctrlkeys = {
    ...
    class VTerm {
        ...
        render_cell(cellx, celly, clear = true) {
            ...
            const vtfy = (vcc >> 4) * 10;
            if (vt_font_bmp[0] != null)
                this.ctx.drawImage(vt_font_bmp[0], vtfx, vtfy, 8, 10, vcx, vcy, 8, 10);
...
mHQBAgDKJ59tAQIatyeiY4BI34AAAAAAAAAA`);
    ...
    const FN_H11 = [
        '', 'RDIN', 'WTIN', 'WH=RS',
        'WCount', 'EACount', 'CROM', 'NibSwap'
    ];
    const FN_E7 = ['', 'BusReady', '', 'BusRead'];
    const FN_D45 = [
        'SwapR', 'RDR', 'EAH', 'EAL', 'SwapR', 'RDR', 'EAH', 'EAL',
    ...
    const oplist = [
        ...
        { n: 'CLRB', md: 11 /* OPM.RC */, w: 0 /* TXS.B */, aop: 0, fl: 8 /* FLAGS.ArithFLMV */,
        ...
        // 0x3H
        { n: ['INR', 'INC'], md: 11 /* OPM.RC */, w: 1 /* TXS.W */, i: 1, fl: 7 /* FLAGS.ArithFMV */ },
        { n: ['DCR', 'DEC'], md: 11 /* OPM.RC */, w: 1 /* TXS.W */, i: 1, fl: 7 /* FLAGS.ArithFMV */ },
        { n: 'CLR', nm: 'CAD', md: 11 /* OPM.RC */, w: 1 /* TXS.W */, fl: 8 /* FLAGS.ArithFLMV */ },
            ...
            function decode_mode(mode, is_write, regm) {
                ...
                switch (mode) {
                    ...
                    case 1 /* OPM.IMM */:
                        opsfx = '=';
                        if (opcs.w == 0 /* TXS.B */) {
                            return (is_write ? litpfx.sw : litpfx.sr) + hexlist(dfetch());
                        }
                        if (opcs.w == 1 /* TXS.W */) {
                            let hi = dfetch();
                            let lo = dfetch();
                            return (is_write ? litpfx.sw : litpfx.sr) + hexlist((hi << 8) | lo, 4);
                        }
                    ...
                    case 6 /* OPM.MOD */: {
                        ...
                        if ((modv & 8) > 0) {
                            disp = sbyte(dfetch());
                            if (name_conv == 1 /* NAMECON.MEI */) {
                                res = `${res}${hexlistp(disp)}`;
                            }
                            else {
                                res = `${res},${hexlist(disp & 255)}`;
                            }
                        }
                        if (name_conv == 1 /* NAMECON.MEI */) {
                    ...
                    case 9 /* OPM.RRSR */: {
                        ...
                        if (opcs.w == 1 /* TXS.W */) {
                            regsrc = regsrc & 14;
                            regdst = regdst & 14;
                            if (mode == 8 /* OPM.RRX */ && !is_cent && (regdst != (regdst | 2)))
                            ...
                            if (mode == 9 /* OPM.RRSR */) {
                                if (is_cent) {
                                    if (rrmode == 2) {
                                        ...
                                        // MSN even
                                        return `${strdst},/${addrrefw(adr)}`;
                                    }
                                    else {
                                        return `${strdst},${regconv[1][regsrc]},/${hexlist(adr)}`;
                                    }
                                }
                                if (rrmode == 2) {
                                    // MSN odd
                                ...
                                if (is_cent) {
                                    if (rrmode == 2) {
                                        ...
                                        return `${regconv[1][regsrc]},${hexlist(adr)},${strdst}`;
                                    }
                                }
                                if (rrmode == 2) {
                                    // MSN odd
                    ...
                    case 11 /* OPM.RC */: {
                        ...
                        if (opcs.w == 1 /* TXS.W */ && (regdst & 1) != 0) {
                            ...
                            regdst = regdst & 14;
                            if (regdst == 0) {
                                return `${DREF_L}${addrref(adr)}${DREF_R}, ${regsrc}`;
                            }
                            return `${DREF_L}${regconv[1][regdst]}${hexlistp(adr, 4)}${DREF_R}, ${regsrc}`;
                        }
                        else {
                            return `${regconv[rsh][regdst]},${regsrc}`;
                        }
                    }
                    case 12 /* OPM.IMPL_R */:
                        if (name_conv != 1 /* NAMECON.MEI */) {
            ...
            function decode_exmode(mode, is_dest, par_len) {
                ...
                switch (mode) {
                    ...
                    case 1:
                        ...
                        if (is_cent)
                            return `-${regconv[1][regh]}${(regl != 0) ? ',' + regconv[1][regl] : ''}${hexlistp(litv)} ; todo syntax`;
                        return `${DREF_L}${regconv[1][regh]}${(regl != 0) ? ' + ' + regconv[1][regl] : ''}${hexlistp(litv)}${DREF_R}`;
                    case 2:
                        if (is_dest) {
                            if (is_cent)
                                return `-${regconv[1][regl]}`;
                            return `${DREF_L}${regconv[1][regl]}${DREF_R}`;
                        }
                        else {
                            exm_data = v0;
                            exm_reg = true;
                            if (is_cent)
                                return `-${regconv[1][regh]}`;
                            return `${DREF_L}${regconv[1][regh]}${DREF_R}`;
                        }
                    case 3:
                        let lits = hex(v0);
                ...
                case 5 /* OPL_EXT.MEM */: {
                    ...
                    if (opcs.sr !== undefined) {
                        ...
                        if ((nci & 2) != 0) {
                            lvlen = dfetch() + 1;
                            if (name_conv == 1 /* NAMECON.MEI */) {
                                pstr = `${name_conv == 1 /* NAMECON.MEI */ ? 'l=#' : litpfx.ir}${lvlen}, `;
                            }
                            else if (is_cent) {
                                pstr = `(${lvlen}`;
                            }
