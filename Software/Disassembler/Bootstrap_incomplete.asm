...
0xfc17:     7B 79            JSR  (PC+0x79)     ; Jump to subroutine at direct PC offset by N address  (0x0092)
0xfc19:     C4 BD 00         "D=\0"     		; String
...
0xfc4f:     D0 83            LDBW #8300         ; Load literal address into byte of BW
0xfc52:     F5 A2            STBW SW, 2         ; Store BW into index mode register (view supplemental material)
0xfc54:     D0 81            LDBW #8100         ; Load literal address into byte of BW
...
0xfc79:     45 15            XFR  AL,        ; Copy byte of one explicit register into other (R into L)
0xfc7b:     03               RF                 ; Reset Fault
0xfc7c:     71 01 03         JMP  0103          ; Jump to direct address
0xfc7f:     7B 11            JSR  (PC+0x11)     ; Jump to subroutine at direct PC offset by N address  (0x0092)
0xfc81:     8D               LALS               ; Load byte from address in S into implicit AL
0xfc82:     8A               LALX               ; Load byte from address in XW into implicit AL
0xfc83:     C5 D2            LDAL, 2         ; Load index mode register address into BL (view supplemental material)
0xfc85:     D2 CF D2         LDBW [CFD2]        ; Load indirect address into byte of BW
0xfc88:     8D               LALS               ; Load byte from address in S into implicit AL
0xfc89:     8A               LALX               ; Load byte from address in XW into implicit AL
...
0xfc9d:     09               RSR                ; Return from Subroutine
...
0xfcb9:     09               RSR                ; Return from Subroutine
...
0xfcca:     D0 00            LDBW #0010         ; Load literal address into byte of BW
...
0xfcd9:     D0 04            LDBW #0400         ; Load literal address into byte of BW
0xfcdc:     5A               NABW               ; AND implicit AW and BW
0xfcdd:     15 A0            BNZ  (PC-0x60)     ; Branch if not equal to zero to 0x60 behind PC (0x007f)
0xfcdf:     D0 00            LDBW #0020         ; Load literal address into byte of BW
...
0xfd10:     09               RSR                ; Return from Subroutine
...
0xfd1b:     09               RSR                ; Return from Subroutine
...
0xfd45:     D0 01            LDBW #0190         ; Load literal address into byte of BW
...
0xfda3:     09               RSR                ; Return from Subroutine
...
0xfdac:     09               RSR                ; Return from Subroutine
...
0xfdea:     40 35            ADD  BL,        ; Add bytes of two explicit registers (L + R stored in L)
...
0xfdf2:     09               RSR                ; Return from Subroutine
...
0xfdfc:     D0 4B            LDBW #4B65         ; Load literal address into byte of BW
0xfdff:     F5