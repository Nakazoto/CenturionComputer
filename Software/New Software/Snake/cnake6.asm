...
RGMOVE    JSR/      MOVESNAKE ; Move the snake.
          ...
          SUB=      MAXSNAKE,A ; Compare to max score.
          BZ        RGWON     ; If equal, set the you-won message.
          MVF       (9)='GAME OVER',/RGMSG ; Set the game-over text.
          JMP       RGPRINT   ; Go to printing it.
RGWON     MVF       (9)='YOU WON!!',/RGMSG ; Set the you-won text.
...
INITBOARD STK       X,2       ; Push X to the stack.
          ...
          JSR/      SETCRT    ; Set the CRT as active.
          MVF       (6)=X'300330043005',/SNAKERC ; Set initial snake rows/cols.
...
MSUP      SRAB                ; A = A >> 1.
          ...
          ADD=      RNEG1,B   ; Subtract one from the row.
          JMP       MSFOOD    ; Check if food is there.
MSDOWN    ADD=      RMULW,B   ; Add one to the row.
...
MSLBRDR   AND=      COLMASK,B,Z ; Z = column.
...
NFRAND    JSR/      RAND      ; Get a random number.
          AND=      RCMASK,B,X ; X = random row+column.
          AND=      COLMASK,B,A ; A = random column.
          LDB/      SCRNWIDTH ; B = screen width.
          DCR       B,2       ; B = B - 2, the width minus borders.
          DIV       B,A       ; A = column within limits.
          INA                 ; A = A + 1, the food column.
          XAB                 ; A -> B.
          SRR       X,RSHFTW  ; X = random row.
          DIV=      24-2,X    ; X = row within limits.
