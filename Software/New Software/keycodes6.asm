* Centurion Keycodes (CPU6) by ren14500. Public domain.
*
...
GAMENAME  JSR/      PRINTNULL ; Print.
 DW CRLF,2
 DC 'KEYCODES ON GOS-6 0.5'
 DW CRLF,3
 DB 0
          RSR                 ; Return.
*
* Output position.
OUTPOS    EQU       (0*256)+6 ; Row 0, column 6.
*
* Run the game. Clobbers everything. Does not return.
RUNGAME   JSR/      DRAWSCRN  ; Clear and draw screen.
RGLOOP1   JSR/      GKANY     ; Wait for input.
          JSR/      HANDLEKEY ; Handle the key pressed.
          JMP       RGLOOP1   ; Keep going forever.
*
* Clear the screen and draw the screen on each CRT. Clobbers all except X.
DRAWSCRN  STK       X,2       ; Push X to the stack.
          CLA                 ; A = 0.
          LDX=      ACTVCRTS  ; X = active CRT table.
          CLRB      YL,1      ; YL = 1.
DSLOOP    LDAB+     X+        ; AL = next active CRT; X = next entry.
          BP        DSGOOD    ; If valid, clear and draw.
          JMP/      DSDONE    ; If <0, no more active CRTs.
DSGOOD    JSR/      SETCRT    ; Set the CRT as active.
          JSR/      CLRS      ; Clear the screen.
          CLR       B         ; B = row 0, column 0.
          JSR/      PRINTNULL ; Print the static text.
          DC        'CODE:'
          DB        0
          LDB=      OUTPOS    ; B = output position.
          JSR/      POSRC     ; Set the cursor position.
          DCRB      YL        ; YL = YL - 1.
          JMP/      DSLOOP    ; Loop for the next CRT.
DSDONE    POP       X,2       ; Pop X from the stack.
          RSR                 ; Return.
*
* Handle the keypress in AU/AL. Clobbers all except X.
HANDLEKEY STK       X,2       ; Push X to the stack.
HKCODE    STA=      0         ; Save the key code.
          LDB=      OUTPOS    ; B = output position.
          JSR/      POSRC     ; Set the cursor position.
          MVF       (2)='#@',/HKSTR1 ; Set the template.
          CLRB      AL,2      ; AL = 2 (length of string).
          LDBB=     '0'       ; BL = ASCII zero (padding byte).
          CFB       /HKSTR1(16),/HKCODE+1(1) ; Convert code to hex.
          MVF       (2)='#@',/HKSTR2 ; Set the template.
          CLRB      AL,2      ; AL = 2 (length of string).
          LDBB=     '0'       ; BL = ASCII zero (padding byte).
          CFB       /HKSTR2(16),/HKCODE+2(1) ; Convert code to hex.
          JSR/      PRINTNULL ; Print the code.
HKSTR1    DB        0,2
          DC        ' '
HKSTR2    DB        0,3
          LDB=      OUTPOS    ; B = output position.
